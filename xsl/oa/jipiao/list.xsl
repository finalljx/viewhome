...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/">
		{
		<xsl:apply-templates select="//div[@id='viewValue']//table//tr[position()&gt;1]" />
		}
	</xsl:template>
	<xsl:template match="tr">"url":"<xsl:value-of select='td[3]//a/@href'/>","title":"<xsl:value-of select='td[4]/.'/>","time":"<xsl:value-of select='td[3]/.'/>"
	</xsl:template>
</xsl:stylesheet>
