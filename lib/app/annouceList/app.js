...
function getAnnouceList(){
	var url = 
	serverUrl+'view/oamobile/annoucement/Application/DigiFlowInfoPublish.nsf/InfoByDateView?readviewentries?login&start=1&count=10';
	$.hori.ajax({
		"type":"get",
		"url":url,
...
function pullUpAction(){
	$.hori.showLoading();
	//alert($("a").attr("data-startNumber"));
	number = parseInt($("a").attr("data-startNumber"))+10;
	var serverUrl=$.hori.getconfig().appServerHost+'view/oamobile/annoucement/Application/DigiFlowInfoPublish.nsf/InfoByDateView?readviewentries?login&start='+number+'&count=10';
	alert(serverUrl);
	setTimeout(function () {
		$.hori.ajax({
			"type":"get",
