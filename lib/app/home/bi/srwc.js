	...
	$.hori.ajax({
		...
		"success" : function(res) {
			...
			$("#rightContent").load("bi/srwc.html", function(a, b, c) {
				...
				var ww = (Math.ceil((seriesdata[0]) / 100)) * 100;
				drawSrwc(ww, srwc, 0.6, 0.7, b + "\n" + c + "\n" + d);
				var ul = document.getElementById("loadBi");
				var lis = ul.getElementsByTagName("li");
				for (var i = 0; i < lis.length; i++) {
					lis[i].style.background = "#ededed";
				}
				document.getElementById("11").style.backgroundColor = '#5e87b0';
			});
		},
		"error" : function(res) {
			alert("error:" + res);
...
function drawSrwc(ww, srwc, colorOne, colorTwo, title) {
	var option = {
			animation : false,
