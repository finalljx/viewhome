            ...
            $.hori.ajax({
        		...
        		for(var i=0;i<busiData.length;i++){
        			...
        			if(30==busiData[i].now){
        				imgString=imgString+"<li style='line-height:50px;'><div style='float:left;clear:both;width:60px;'><font size='2'>"+busiName+"</font></div><div style='float:left;width:50px;'><img src='bi/img/dahong.png'></img></div>"
        			}else {
        				imgString=imgString+"<li style='line-height:50px;'><div style='float:left;clear:both;width:60px;'><font size='2'>"+busiName+"</div><div style='float:left;width:50px;'><img src='bi/img/dalv.png'></img></div>"
        			}
        			var imageType=busiData[i].imageType.split('|');
        			for(var j=0;j<imageType.length;j++){
        				if(10==imageType[j]){
        					imgString=imgString+"<div style='float:left;width:6%;line-height:60px;'><img src='bi/img/xiaolv.png'></img></div>";
        				}else if(20==imageType[j]){
        					imgString=imgString+"<div style='float:left;width:6%;line-height:60px;'><img src='bi/img/xiaohuang.png'></img></div>";
        				}else if(30==imageType[j]){
        					imgString=imgString+"<div style='float:left;width:6%;line-height:60px;'><img src='bi/img/xiaohong.png'></img></div>";
        				}else{
        					imgString=imgString+"<div style='float:left;width:6%;line-height:60px;'><img src='bi/img/xiaohui.png'></img></div>";
        				}
        			}
				...
				for (var i = 0; i < lis.length; i++) {
                 ...
                 document.getElementById("21").style.backgroundColor='#836FFF'; 
               },
                "error":function(res){
                    alert("error:" + res);
                    $.hori.hideLoading();
