		...
		$.hori
				.ajax({
					...
					"success" : function(res) {
						...
						var widthImg2 = document.documentElement.clientWidth * 0.43;
						...
						if (!(r.test(res))) {
							res = res.replace(/CACHEID=.*?\\/g,
									'data-result=file&data-application=' + key
											+ '\\\" width=\\\"' + widthImg
											+ '\\');
						...
						$("#rightContent")
								.load(
										...
										function(a, b, c) {
											document
													.getElementById('rightContent').style.dispaly = 'dispay';
											$.hori.hideLoading();
											if (data.attachmentSource != undefined) {
												document
														.getElementById("fujian").style.display = "inline";
												...
												var dataArrayName = data.attachment
														.split("|");
												...
												document
														.getElementById("fujian").style.display = "none";
											}
											renderDetail(data);
											...
											/*
											 * var a=$('.details
											 * table:first').html(); var r=/<\s*p.*?>(.*)<\s*\/p\s*>/g;
											 * a=a.replace(r,'$1'); $('.details
											 * table:first').html(a);
											 */

											/*
											 * var a=$('.details
											 * table:last').html(); var r=/<\s*p.*?>(.*)<\s*\/p\s*>/g;
											 * a=a.replace(r,'$1'); $('.details
											 * table:last').html(a);
											 */
