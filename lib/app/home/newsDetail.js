...
function getWcmNewsData() {
	...
	$.hori.ajax({
		...
		"success" : function(res) {
			...
			res=res.replace(/CACHEID=.*?\\/g,'data-result=file&data-application='+key+'\\\" width=\\\"'+widthImg+'\\');
			...
			$("#rightContent").load("news/newsDetail.html", function(a, b, c) {
				...
				if (data.attachmentSource!=undefined) {
					...
					for (var i = 0; i < dataArrayName.length; i++) {
						vvar lastIndex = dataArrayName[i].lastIndexOf(".");
						var cut = dataArrayName[i].substring(lastIndex);
...
function viewfile(item){
	...
	var url=item.attachmentSource()+"&data-convert=office&data-basic-auth-u=wpsadmin&data-basic-auth-p=GTAdmin";
	url=url.replace(/\&amp;/g,"&");
	console.log(item.attachmentSource());
	localStorage.setItem("attachmentUrl",url);
	$.hori.loadPage( $.hori.getconfig().serverBaseUrl+"viewhome/html/attachmentShowForm4News.html", $.hori.getconfig().serverBaseUrl+"viewhome/xml/AttachView.xml");
}
