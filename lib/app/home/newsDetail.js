			...
			$("#rightContent").load("news/newsDetail.html", function(a, b, c) {
				if (null != data.attachmentSource) {
					...
					if(data.attachment.charAt(attachmentNameLength - 1) === "|"){
						data.attachment = data.attachment.substring(0,attachmentNameLength -2);
					}
					if(data.attachmentSource.charAt(attachmentSourceLength - 1) === "|"){
						data.attachmentSource = data.attachmentSource.substring(0,attachmentSourceLength - 2);
					}
					...
					for(var i=0; i<dataArraySource.length;i++){
						dataArraySource[i]=$.hori.getconfig().appServerHost+'view/oa/file/'+dataArraySource[i];
