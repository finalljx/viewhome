function getTpxwNewsList(itcode, type,pageSize,pageNow) {
...
function render(jsonData,type) {
	...
	case "GT_HomePage_SiteArea_jttz":
		...
		$('#jttzList').prepend('<li data-role="list-divider" role="heading" class="ui-li ui-li-divider ui-bar-b ui-first-child"><a class="ui-link">通知公告</a> <a href="#" style="float: right" class="ui-link" onclick="loadJttzList();">more</a></li>');
		break;
	case "GT_HomePage_SiteArea_jtxw":
		ko.applyBindings(viewModel,document.getElementById("jtxwList"));
		$('#jtxwList').prepend('<li data-role="list-divider" role="heading" class="ui-li ui-li-divider ui-bar-b ui-first-child"><a class="ui-link">集团新闻</a> <a href="#" style="float: right" class="ui-link" onclick="loadJtxwList();">more</a></li>');
		break;
	case "GT_HomePage_SiteArea_zgsxw":
		ko.applyBindings(viewModel,document.getElementById("cydwxwList"));
		$('#cydwxwList').prepend('<li data-role="list-divider" role="heading" class="ui-li ui-li-divider ui-bar-b ui-first-child"><a class="ui-link">成员单位新闻</a> <a href="#" style="float: right" class="ui-link" onclick="loadCydwxwList();">more</a></li>');
		break;
	}
}
//图片新闻
function showDetail(item){
	...
	localStorage.setItem("currentGT_HomePage","GT_HomePage_SiteArea_tpxw");
	$.hori.showLoading();
...
function showDetailJttz(item){
	...
	alert(contentName);
	localStorage.setItem("currentContentName",contentName);
	localStorage.setItem("currentGT_HomePage","GT_HomePage_SiteArea_jttz");
	$.hori.showLoading();
	$.hori.loadPage($.hori.getconfig().serverBaseUrl+ 'viewhome/html/news/newsDetail.html');
}
//集团新闻
function showDetailJtxw(item){
	...
	alert(contentName);
	localStorage.setItem("currentContentName",contentName);
	localStorage.setItem("currentGT_HomePage","GT_HomePage_SiteArea_jtxw");
	$.hori.showLoading();
	$.hori.loadPage($.hori.getconfig().serverBaseUrl+ 'viewhome/html/news/newsDetail.html');
}
//成员单位新闻
function showDetailZgsxw(item){
	...
	alert(contentName);
	localStorage.setItem("currentContentName",contentName);
	localStorage.setItem("currentGT_HomePage","GT_HomePage_SiteArea_zgsxw");
	$.hori.showLoading();
	$.hori.loadPage($.hori.getconfig().serverBaseUrl+ 'viewhome/html/news/newsDetail.html');
}
//初始化swiper
