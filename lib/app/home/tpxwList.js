...
$(document).ready(function() {
	document.getElementById("tabbox").style.display = 'block';// 更改ipad页面中tab
	// 是否显示。
				...
				try {
					...
					$.hori
							.ajax({
								...
								// "headers" : {
								// "Authorization" : "Basic
								// d3BzYWRtaW46R1RBZG1pbg=="
								// },
								"success" : function(res) {
									res = res.replace(/\?/g, '');
									var list = JSON.parse(res);
									for ( var item in list) {
										list[item].tpxwImage = list[item].tpxwImage
												.substring(list[item].tpxwImage
														.indexOf(".cn") + 3);
										var targetURL = $.hori.getconfig().appServerHost2
												+ 'view/tpxw/file'
												+ list[item].tpxwImage
												+ "?data-result=file&data-application="
												+ $.hori.getconfig().appKey
												+ "&data-thumb=200";
										;
										list[item].tpxwImage = targetURL;
									}
									...
									if (page == 1) {
										...
										localStorage.setItem(
												"currentContentName",
												jsonData.list[0].contentName);
										document
												.getElementById("middleContent").style.width = '43%';
...
function showDetail(item, moreCurrentName) {
	...
	$.hori.loadPage($.hori.getconfig().serverBaseUrl
			+ 'viewhome/html/news/newsDetailxq.html');
