	...
	$.hori.ajax({
		...
		"success" : function(res) {
			...
			if (json.length > 0) { // 有列表数据返回
				...
				localStorage.setItem("aqsczbb", json[0].OA_MOBILE);
				...
				localStorage.setItem("aqsczbb", 'N');
				hidedoc();
				// 初始化页面内容
				initPage();
			}
...
function getCNName() {
	...
	$.hori.ajax({
		"type" : "get",
		"url" : $.hori.getconfig().appServerHost + "/view/oa/currentuser/"
				+ docServer + "/genertec/persontasks.nsf/frmPage?openform",
		"success" : function(res) {
...
function createUser() {
	...
	$.hori.ajax({
		...
		"success" : function(res) {
			//localStorage.setItem("Chinesename", res);
			localStorage.setItem("Chinesename", "赵杰");
			currentUser = "CN=" + res + "/O=genertec";
			localStorage.setItem("itcodetasklist", currentUser);
			$.hori.hideLoading();
		},
...
function initPage() {
	...
	var flag = true;
	var target = [];
	for(var i=0; i<disableModules.length; i++){
		console.log(disableModules[i]);
	}
