...
function clearECharts(biId){
	if(myChart){//存在myChart对象
		console.log("清除echarts缓存");
		if(biId != 1 && biId != 21){ //不为经营态势和风险状况时，释放图表实例
			myChart.clear();
			myChart.dispose();
		}
	}
...
function loadXgbb() {
	...
	$("#rightContent").empty();
	$("#middleContent").empty();
	$("#middleContent").load("xgbb.html", function(a, b, c) {
		document.getElementById("middleContent").style.width = '90%';
	});
}
// 加载请示查阅
