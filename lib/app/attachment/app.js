...
app.render=function(jsonObj){
	...
	var serverUrl=config.appServerHost2;
	total=jsonObj.total;
	path=jsonObj.path;
	...
	if(type=="path"){
		...
		if(path.indexOf("?")!=-1){
			...
			path =serverUrl+path + "?data-random=" + Math.random();
			path = path.replace("&data-application="+$.hori.getconfig().appKey,"");
		}
			...
			if($("#imgcontent")){
			    ...
			    var path= path.replace('\\?', '&');*/
			    console.log(path);
			    //path='http://192.168.1.150:8080/viewhome/tmp/5bc1a6857a9049c3af8c48cce74e6b71/RGIj9KPCaJGY+tASdBGwA.jpg?data-application='+$.hori.getconfig().appKey+'&data-random=0.99675810826011 ';
...
$(document).ready(function(){
    ...
    attachUrl = attachUrl.replace($.hori.getconfig().appServerHost2,$.hori.getconfig().appServerHost);
...
function changeimg(){
	try{
		...
		if(path.indexOf('?')==-1){
			...
			var imgSrcPath=path + "?data-page=" + currentPage+"&data-cache=true&data-random=" + random+"&data-userstore="+dataUserstore
		}else{
			var imgSrcPath=path + "&data-page=" + currentPage+"&data-cache=true&data-random=" + random;
		}
