...
app.render=function(jsonObj){
	...
	if(type=="path"){
		...
		if(path.indexOf("?")!=-1){
			...
			path =serverUrl+path + "?data-random=" + Math.random();
			path = path.replace("&data-application=mobile","");
		}
		
			if($("#imgcontent")){alert(path);
			    /*var path= path.replace('\\&', '?');
			    var path= path.replace('\\?', '&');*/
			    console.log(path);
			    //path='http://192.168.1.150:8080/viewhome/tmp/5bc1a6857a9049c3af8c48cce74e6b71/RGIj9KPCaJGY+tASdBGwA.jpg?data-application=mobile&data-random=0.99675810826011 ';
