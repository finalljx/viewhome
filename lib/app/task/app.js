...
function gettaskList(itcode){
	$.hori.showLoading();
	var serverUrl=$.hori.getconfig().appServerHost+"view/oa/todos/doctest/genertec/persontasks.nsf/dbview?openform&view=vwgwdbshow&restricttocategory="+itcode+"&start="+pages+"&count=10";
	//alert(serverUrl);
	setTimeout(function () {
		$.hori.ajax({
			"type":"get",
	...
	$.hori.ajax({
			...
			"success" : function(res) {
				...
				if (window.navigator.userAgent.match(/iPad/i) || window.navigator.userAgent.match(/iPhone/i) || window.navigator.userAgent.match(/iPod/i)||window.navigator.userAgent.match(/android/i)){
					...
					var url= 'viewhome/oa/'+template+'/doctest/'+docunid+'?editdocument&data-application=' + $.hori.getconfig().appKey;
				}
